            ...
            <div class="flex items-center px-4">
                <div class="flex flex-col items-center justify-center gap-2">
                    <a
                        ...
                        class="mr-6 w-full text-sm font-medium text-blue-900 dark:text-white hover:underline"
                        >{{ 'Call: ' + phone_number }}</a
                    >
            ...
            <div class="flex items-center">
                <ul class="flex flex-row mt-0 mr-6 space-x-8 text-sm font-medium">
                    <li v-for="i in Object.keys(menu_items)">
                        <button
                            v-if="!(menu_items[i].constructor == Object)"
                            class="text-white dark:text-white hover:underline"
                            @click="hook_of_other_element(menu_items[i], i)"
                        >
                            {{ i }}
                        </button>
                        <div v-else>
                            ...
                            <div
                                ...
                                :ref="
                                    el => {
                                        drop_box_store[i] = { el: el, is_shown: 0 };
                                    }
                                "
                            ...
                            >
                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-400">
                                    <li v-for="x in Object.keys(menu_items[i])">
                                        <button
                                            @click="hook_of_other_element(menu_items[i][x], x)"
...
function show_hide(name) {
    ...
    if (drop_box_store[name]['is_shown'] == 0) {
        ...
        setTimeout(() => {
            drop_box_store[name]['el'].classList.add('bounce-leave');
        }, 100);
        setTimeout(() => {
            drop_box_store[name]['el'].style.display = 'none';
        }, 470);
    } else {
