    ...
    <div class="flex h-full w-full flex-col">
        <div v-if="!no_child.state" class="flex flex-row mt-5 w-full justify-center items-center">
            <p class="text-center text-base text-blue-700 dark:text-gray-200">
                Contact us (email/phone) for further information. Prices may vary based on requirements.
            </p>
        </div>
        <div class="flex flex-col h-full min-w-[100%] justify-center items-start flex-nowrap overflow-auto">
            ...
            >
                <TransitionGroup :css="false" appear @enter="enter" @after-enter="end">
                    <div
                        v-for="i in data[target]"
                        :key="i"
                        :id="data[target].indexOf(i)"
            ...
            >
                ...
                >
                    <h1 class="text-left mb-4 text-5xl font-extrabold leading-none tracking-tight text-blue-200">Contact Us.</h1>
...
if (data[target].length == 1) {
    ...
    animation_delay = calculate_animation_delay(0.3);
    reverse_animation_delay = calculate_reverse_animation_delay(animation_delay);
}

const enter = (el, done) => {
    if (animation_delay[el.id] == '~') {
        // differentiate the middle element
...
function end(el) {
    let delay = reverse_animation_delay[el.id] == '~' ? 0.1 : Math.abs(reverse_animation_delay[el.id]) + 0.7;
...
function fade(element, delay, done) {
    gsap.fromTo(element, { opacity: 0 }, { opacity: 0.999, delay: delay, duration: 0.5, onComplete: done, lazy: false });
}
